...
`⌘8`은 이 저장소를 공유하는 모든 체크아웃을 보여줍니다 — 메인 클론과 `git worktree add`로 만든 것 전부. 다른
디렉터리에 두고 온 브랜치를 터미널을 하나 더 열지 않고 한 키로 엽니다. 각 행은 따로 찾아봐야 알 수 있던 상태를
그대로 답니다: 브랜치, HEAD 커밋이 무슨 일을 하고 있었는지, 커밋 안 된 작업이 있는지, upstream에서 얼마나
벌어졌는지. Enter를 누르면 그 워크트리가 자기 창으로 열리고, 이미 떠 있는 워크트리는 새로 열지 않고 포커스만
옮깁니다.

[`gh`](https://cli.github.com)가 설치돼 있고 리모트가 GitHub이면, 열린 PR이 있는 브랜치 행에 그 번호도 함께
붙습니다 — 시작할 때는 로컬 커밋뿐이던 워크트리가 나중에 PR이 올라오면 찾아보지 않아도 스스로 그렇게 말합니다.
kakapo가 GitHub에 닿는 건 이 읽기 하나뿐이고, 무엇도 올리지 않습니다. 리뷰 코멘트는 워크트리별로 `.git/` 안에
로컬로 남습니다 — PR이 생기기 전에 쓴 코멘트가 생긴 뒤에도 그대로인 이유는, 애초에 PR에 매여 있지 않았기
때문입니다.

kakapo는 워크트리를 나열할 뿐, 만들거나 옮기거나 지우지 않습니다. `git worktree add`는 여전히 당신이 직접
실행합니다.
