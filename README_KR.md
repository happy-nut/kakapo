...
코딩 에이전트는 빠릅니다. 그 결과물을 읽는 쪽이 느리죠. Kakapo는 그 반쪽을 위해 만들어졌습니다 — 진짜 diff, 진짜 language server 탐색, 에이전트가 그 자리에서 답하는 리뷰 코멘트, 공유 지식 그래프, worktree별 마크다운 메모.

![Kakapo에서 worktree를 만들고, 라인 코멘트로 diff를 리뷰하고, 지식 그래프를 살펴보고, 마크다운 메모를 작성하는 화면](assets/kakapo-core-flow.gif)
...
**코멘트를 쓰면 답변이 그 라인 위로 자동으로 돌아옵니다.** 라인에서 `?`를 눌러 질문이나 변경 요청을 남깁니다. 저장하는 즉시 Kakapo의 읽기 전용 리뷰 에이전트가 답변을 시작하고, 진행 상태가 코멘트 아래에 나타나며, 답변은 같은 스레드에 붙습니다. 합본 프롬프트를 만들거나 전송할 단계는 없습니다. `F8`로 열린 스레드를 끝까지 훑습니다.

**에이전트 프롬프트가 diff를 설명하고 낯선 코드베이스의 지도를 만듭니다.** `⌘⇧P`로 프롬프트 팔레트를 엽니다. *diff 설명*은 중요한 라인에 쉬운 말로 노트 카드를 달고 짧은 브리핑을 엽니다. `F8`로 카드를 따라가고 `⌘⇧B`로 브리핑을 다시 봅니다. *코드베이스 설명*은 상위 구조를 담은 지도 하나를 남깁니다. 이 작업은 Kakapo의 백그라운드 에이전트 세션에서 실행되고, 노트는 `.git/kakapo/knowledge.jsonl`에 누적되어 모든 worktree가 공유합니다.

**리뷰 대화가 지식 그래프로 남습니다.** `⌘⇧P`의 *배운 말 남기기*는 현재 Claude·Codex 대화에서 아직 읽지 않은 부분만 보고, 리뷰어가 실제로 받아들인 개념을 남깁니다. `⌘⇧K`로 그래프를 열면 단어들이 의미를 통해 연결되고, 각 개념이 코드의 식별자와 위치를 가리킵니다. 설정에서 Kakapo MCP 서버를 연결하면 터미널 에이전트도 어떤 대화에서든 같은 그래프를 읽고 확장할 수 있습니다. 그래프는 모든 worktree가 공유하는 `.git/kakapo/terms.jsonl`에 저장됩니다.

**worktree마다 마크다운 메모가 하나씩 있습니다.** `⌘⇧N`으로 리뷰 판단과 다음 할 일을 적는 집중형 편집 화면을 엽니다. 저장소 밖 Kakapo 앱 데이터 디렉터리에 자동 저장됩니다.

**워크스페이스 하나, 작업 하나당 worktree 하나.** `⌘N`은 `~/kakapo/workspaces/<repo>/<task>`에 관리되는 worktree를 만들고, base 브랜치를 먼저 fetch하고, 원하면 `claude`나 `codex`를 바로 그 안에서 실행합니다. 다른 워크스페이스로 옮겨도 에이전트는 계속 실행되고, tmux가 설치되어 있으면 앱을 껐다 켜도 살아 있습니다. 왼쪽 rail이 어느 워크스페이스가 작업 중이고 어느 쪽이 나를 기다리는지 배지로 알려주고, 다른 워크스페이스에서 턴이 끝나면 시스템 알림이 옵니다. `⌘⌥1–9`로 즉시 전환합니다.
...
**추적되는 프로젝트 파일에는 아무것도 쓰지 않습니다.** 리뷰 스레드, 설명 노트, 지식 그래프는 `.git/kakapo/`에 있습니다 — git은 자기 디렉터리를 추적하지 않으니 `git status`는 깨끗하고, cwd에 갇힌 코딩 에이전트도 그 파일에 닿을 수 있습니다. 나머지 상태는 워크스페이스 절대 경로별로 OS 애플리케이션 데이터 디렉터리에 격리됩니다. 전부 평문 JSONL/Markdown/JSON, 완전 로컬, 계정도 텔레메트리도 없고 MIT입니다.

## 리뷰 루프

1. 워크스페이스 터미널(``⌃` ``)에서 에이전트가 작업합니다.
2. 실제 diff를 읽습니다 — `F7`로 hunk 이동, `Space`로 파일 확인 표시.
3. 라인에서 `?`로 질문하거나 변경을 요청하면 저장 즉시 리뷰 에이전트가 자동으로 답변합니다.
4. 답변은 같은 스레드에 붙고, `F8`로 따라갑니다.
5. 에이전트 설명은 `⌘⇧P`, 지식 그래프는 `⌘⇧K`, worktree 메모는 `⌘⇧N`으로 엽니다.
...
리뷰 스레드, 누적된 설명 노트, 지식 그래프는 저장소의 git 디렉터리 안에 있습니다.

```text
.git/worktrees/<name>/kakapo/comments.jsonl   # 이 워크스페이스의 리뷰 대화
.git/kakapo/knowledge.jsonl                   # 모든 worktree가 공유하는 에이전트 설명 노트
.git/kakapo/terms.jsonl                       # 모든 worktree가 공유하는 지식 그래프
```
