...
## 워크스페이스 허브와 관리형 worktree

- 다시 실행한 `kakapo`는 실행 중인 Electron 인스턴스에 합류한다.
- 같은 checkout의 하위 경로는 한 워크스페이스로, linked worktree는 별개로 식별한다.
- 단일 shell 창이 repo별 허브와 워크스페이스별 격리 `WebContentsView`를 소유한다.
- `Cmd/Ctrl+K`, `Cmd/Ctrl+N`, `Cmd/Ctrl+Alt+1–9`로 허브, 생성, 직접 전환을 제어한다.
- 생성은 base를 감지하고 main loop를 막지 않고 fetch하며, 실패하면 로컬 base와 경고로 진행한다.
- 삭제는 미커밋, unpushed, 실행 중 프로세스를 경고하고 브랜치 삭제는 별도 옵트인이다.
- 열린 목록, 별칭, 메모, 활성 항목, 끊긴 경로, 에이전트 resume 정보가 재시작 후 복원된다.
- 비활성 워크스페이스는 PTY를 유지하고 idle 뒤 분석 자원을 해제한다.
...
코멘트들은 `Cmd/Ctrl+Shift+/`로 여는 하나의 통합 병합 프롬프트에 질문 섹션이 먼저, 변경 요청 섹션이
그다음에 오도록 모여, 파일·줄 출처와 함께 에이전트에게 돌려보낼 수 있어야 한다.
