...
있다. 에이전트는 사용자의 터미널에서 돌고, kakapo는 그 결과를 읽는 쪽만 맡는다. 이 디렉토리의 테스트는 그 **핵심 유저 플로우**가 앞으로의 모든 배포에서 깨지지 않도록 지킨다.

## 창과 저장소

- 같은 저장소에서 다시 실행한 `kakapo`는 이미 그것을 리뷰 중인 창으로 이동한다.
- 같은 checkout의 하위 경로는 한 창으로, linked worktree는 별개 창으로 식별한다.
- 최소화된 창은 폴링을 멈추고, idle이 지속되면 분석 자원을 해제한다.
- 창은 화면을 꽉 채운 채로 열리고, 파일 트리는 첫 페인트부터 펼쳐져 있다.

## 무엇을 비교하는가 (`compare-menu.test.mjs`)

툴바 pill이 지금 비교 중인 것을 말하고, 그 자리에서 바꿀 수 있어야 한다.

- `⌥A` 이 브랜치의 모든 변경 / `⌥U` 아직 커밋하지 않은 것만. 토글 하나가 아니라 키 두 개다 —
  토글은 지금 어느 쪽인지 알아야 쓸 수 있고, 그걸 모르는 게 이 키를 누르는 이유다.
- `⌥C`는 드롭다운을 브랜치 목록에서 열고 검색창에 포커스를 준다. 저장소 기본값에는 표시가 붙는다.
- "모든 변경사항"은 브랜치 tip이 아니라 merge-base 기준이다. 그러지 않으면 그사이 움직인 `main`의
  남의 커밋이 내가 지운 것으로 보인다.
- 고른 브랜치는 저장소별로 기억된다. 어떤 브랜치와 비교할 가치가 있는지는 그 저장소의 속성이다.
...
- 한 줄의 코멘트는 하나다. 이미 코멘트가 있는 줄에서 다시 열면 새로 쌓이지 않고 그 코멘트를 편집한다
- 자동 포커스는 화면에 보이는 컴포저에 간다(숨은 중복이 아니라)
- 질문과 변경요청은 하나의 종류다 — 어떤 스레드든 충분히 길어지면 "그러니 바꿔줘"에 닿기 때문
...
열린 코멘트들은 `Cmd/Ctrl+Shift+/`로 여는 하나의 통합 문서에 리뷰 순서대로 모여, 파일·줄 출처와 함께
에이전트에게 돌려보낼 수 있어야 한다. 한 항목은 출처 한 줄 · 본문 · 빈 줄이다. 출처는 한 번만 적힌다 —
본문에 프리필된 앵커가 그대로면 본문 쪽은 지운다. `###`도 `#N`도 붙지 않는다: 파일 참조는 제목이 아니고,
번호는 에이전트 카드까지 세는 내부 시퀀스라 문서상의 순번과 맞은 적이 없다.
...
- macOS의 전체 화면 상단 surface(접힌 사이드바의 리뷰 툴바·History·최대화한 합본 문서)는 하나의 공통 safe-area 규칙을 사용해 좌상단 창 버튼을 침범하지 않는다
...
- '반영된 듯' 코멘트는 다음 합본 문서에서 기본 제외되어 매 라운드가 열린 항목만 싣고, 재열기하면 다시 포함된다.
- 에이전트가 답한 스레드도 같은 이유로 빠진다. 리뷰어의 후속 발언이라는 것이 없으므로(스레드당 하나),
  답을 받은 스레드는 그대로 조용해진다.
...
- `.github/workflows/release.yml` — `v*` 태그 배포(Linux tarball + macOS dmg + Windows zip) 직전 게이트
- `.github/workflows/auto-release.yml` — main 머지가 곧 릴리스인 자동 배포에서 version bump 직전 게이트
