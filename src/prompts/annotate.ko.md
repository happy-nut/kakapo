...
각 노트를 쓰는 방법. 규칙은 여덟 개뿐이고, 전부 같은 곳을 가리킵니다 — **짧게, 쉽게**:
...
4. **새 용어는 한 노트에 하나까지.** 디바운스, 레이스 컨디션 같은 진짜 이름은 그대로 쓰되 처음 나올 때 반 문장으로 값을 치르세요. 이 저장소가 만들어낸 이름(pin, manifest 같은)은 독자가 찾아볼 곳이 없으니 반드시 그 자리에서 정의하고, 반 줄로 정의 못 하면 아직 쓸 때가 아닙니다.
...
8. **비유는 짧을 때만.** 한 줄로 이해를 앞당기면 쓰고, 설명이 필요한 비유면 버리세요.
