...
// profiler internals and writes plain JSON under the reviewed repository, so regressions can be compared
// without a hosted telemetry service. The bounded event list prevents a long-running watch session from
// growing the artifact forever.
export class ReviewPerformanceTrace {
  ...
  private writeLatest(): void {
    try {
      const dir = join(this.root, ".monacori", "perf");
