...
function pickAgent(id){const found=AGENTS.find(a=>a.id===id)||AGENTS[0];document.querySelector("#agent").value=found.id;document.querySelector("#agentName").textContent=found.label;closeAgentMenu();}
agentMenu.innerHTML=AGENTS.map(a=>'<button type="button" role="option" data-agent="'+a.id+'"><span class="pm-name">'+esc(a.label)+'</span></button>').join('');
