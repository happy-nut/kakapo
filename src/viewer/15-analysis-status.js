...
// ---- Sidebar-footer usage quota widgets: one row per limit window that can actually stop work — Claude's 5h
// session, its weekly all-models cap and any per-model weekly cap, then Codex's — each a battery of the quota
// still LEFT plus how long until that window resets. Data from kakapoUsage (Claude via the OAuth usage API,
// Codex from its session logs). Numbers and glyphs are app-provided; the model label comes from the API, so it
// is escaped. Marks are the official Claude and OpenAI logos (simple-icons, CC0). ----
...
function usageFmtSpan(d) {
  var h = Math.floor(d / 3600000), days = Math.floor(h / 24), rh = h % 24, m = Math.floor((d % 3600000) / 60000);
  if (days > 0) return days + t('usage.unit.d') + ' ' + rh + t('usage.unit.h');
  if (h > 0) return h + t('usage.unit.h') + ' ' + m + t('usage.unit.m');
  return m + t('usage.unit.m');
}
function usageFmtReset(ms) { var d = ms - Date.now(); return (!ms || d <= 0) ? t('usage.now') : usageFmtSpan(d); }
// Battery of the quota still left — a pie of "remaining" reads like a pie of "used"; a battery doesn't. Amber
// under a quarter left, red under a tenth, so a nearly-flat window is obvious without reading the number.
function usageBattery(pct, color) {
  var p = Math.max(0, Math.min(100, pct));
  var fill = p <= 10 ? '#e5484d' : p <= 25 ? '#f5a524' : color;
  return '<svg class="usage-batt" viewBox="0 0 22 12" aria-hidden="true">'
    ...
    + (p > 0 ? '<rect class="usage-batt-fill" x="2.2" y="3.1" width="' + (13.6 * p / 100).toFixed(2) + '" height="5.8" rx="1.2" fill="' + fill + '"/>' : '')
    + '</svg>';
}
// One limit window: [provider mark] [window] [battery] [% left] [time to reset]. The mark is drawn on the
// provider's first row only, so stacked windows read as one group.
function usageRow(kind, ico, color, label, pct, resetsAt, tooltip) {
  var el = document.createElement('span');
  el.className = 'usage-row usage-row-' + kind;
  el.title = tooltip;
  el.innerHTML = '<span class="usage-row-ico">' + (ico || '') + '</span>'
    + '<span class="usage-label">' + escapeHtml(label) + '</span>'
    + usageBattery(pct, color)
    + '<span class="usage-pct">' + Math.round(pct) + '%</span>'
    + '<span class="usage-reset">' + escapeHtml(usageFmtReset(resetsAt)) + '</span>';
  return el;
}
function usageWindowName(limit) { return limit.label || t(limit.kind === 'session' ? 'usage.session' : 'usage.weekly'); }
function usageTip(provider, name, pct, resetsAt) {
  return provider + ' · ' + name + '\n' + Math.round(pct) + '% ' + t('usage.left') + ' · ' + t('usage.resets') + ' ' + usageFmtReset(resetsAt);
}
function renderUsageFoot() {
  ...
  Promise.resolve(window.kakapoUsage.get()).then(function (s) {
    ...
    if (!s) return;
    if (s.claude) {
      // Worst-first from the main process: the window closest to its cap is the one that decides when work stops.
      var c = s.claude, lims = c.limits || [], first = true;
      // A quota kept through a rate-limited refresh still answers "how much is left" — say how old it is.
      var age = c.quotaAt && Date.now() - c.quotaAt > 120000 ? '\n' + t('usage.asOf').replace('{age}', usageFmtSpan(Date.now() - c.quotaAt)) : '';
      lims.forEach(function (l) {
        var name = usageWindowName(l);
        el.appendChild(usageRow('claude', first ? CLAUDE_ICO : '', '#d97757', name, 100 - l.usedPercent, l.resetsAt, usageTip('Claude', name, 100 - l.usedPercent, l.resetsAt) + age));
        first = false;
      });
      if (!lims.length && c.tokensToday) {
        var tokens = document.createElement('span');
        tokens.className = 'usage-row usage-row-claude';
        tokens.title = 'Claude\n' + usageFmtTokens(c.tokensToday) + ' ' + t('usage.tokensToday');
        tokens.innerHTML = '<span class="usage-row-ico">' + CLAUDE_ICO + '</span><span class="usage-label">' + t('usage.tokensToday') + '</span>'
          + '<span class="usage-pct usage-pct-tokens">' + usageFmtTokens(c.tokensToday) + '</span>';
        el.appendChild(tokens);
      }
    }
    if (s.codex && s.codex.primary) {
      var plan = 'Codex' + (s.codex.planType ? ' (' + s.codex.planType + ')' : '');
      // Codex reports its own window lengths, so name each from windowMinutes rather than assuming an order.
      [s.codex.primary, s.codex.secondary].forEach(function (w, i) {
        if (!w) return;
        var name = t(w.windowMinutes >= 1440 ? 'usage.weekly' : 'usage.session');
        el.appendChild(usageRow('codex', i === 0 ? CODEX_ICO : '', '#10a37f', name, 100 - w.usedPercent, w.resetsAt, usageTip(plan, name, 100 - w.usedPercent, w.resetsAt)));
      });
    }
  }).catch(function () {});
}
