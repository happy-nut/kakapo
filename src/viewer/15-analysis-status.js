...
// ---- Sidebar-footer usage quota widgets (compact): Claude + Codex, each a brand mark + a battery of the quota
// still LEFT. Data from kakapoUsage (Claude's limits via the OAuth usage API, Codex from its session logs).
...
function usageItem(kind, ico, color, pct, valText, tooltip) {
  ...
  el.innerHTML = ico + (typeof pct === 'number'
    ? usageBattery(pct, color) + '<span class="usage-pct">' + Math.round(pct) + '%</span>'
...
function renderUsageFoot() {
  ...
  Promise.resolve(window.kakapoUsage.get()).then(function (s) {
    ...
    if (!s) return;
    if (s.claude) {
      // limits arrive worst-first, so [0] is the window that decides when work actually stops.
      var c = s.claude, lims = c.limits || [];
      var left = lims.length ? 100 - lims[0].usedPercent : undefined;
      var tip = 'Claude';
      lims.forEach(function (l) { tip += '\n' + l.label + ': ' + Math.round(100 - l.usedPercent) + '% left · resets ' + usageFmtReset(l.resetsAt); });
